...
class SignupZoo extends StatefulWidget {
  final Size size;
  final Function(dynamic retValue) onClose;
  final Function(bool value) setBusy;
  SignupZoo({Key key, this.size, this.onClose, this.setBusy});

  SignupZooState createState() => SignupZooState();
}

class SignupZooState extends State<SignupZoo> {
  SignupZooState();
  ...
  _onPrivacyTap() async {
    if (await canLaunch(Utils.privacyTerms)) {
      ...
      throw 'Could not launch privacyTerms';
    }
  }

  _onTkHelpTap() async {
    if (await canLaunch(Utils.tkHelp)) {
      await launch(Utils.tkHelp);
    } else {
      throw 'Could not launch tkHelp';
    }
  }

  _onCountryChanged(int val) {
    print("selected Country: "+_selectedCountryListItem.toString());
    setState(() {
      _selectedCountryListItem = val;
    });
    // if (cmbCountry.selectedItem.data == -1) return;
    //
    // txtPostalCode.text = "";
    // if (!ZPartnersConfig.instance.allowCountryPostCodes( cmbCountry.selectedItem.data )) {
    // lblPostalCode.text = getResourceBundle("lblCity");
    // txtPostalCode.maxChars = 40;
    // txtHelp.visible = false;
    //
    // } else {
    // lblPostalCode.text = getResourceBundle("lblPostalCode");
    // txtPostalCode.maxChars = 5;
  ...
  void didChangeDependencies() {
    if (!_inited) {
      ...
      print("_selectedCountryListItem : "+_selectedCountryListItem.toString());
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  _selectedCountryListItem == -1 ? Container( width: widget.size.width * 0.3, height: 80) : _selectedCountryListItem == 0 ? Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Padding(
                          padding: const EdgeInsets.only(left: 10),
                          child: Text(
                            AppLocalizations.of(context).translate("app_signup_lblCity"),
                            style: TextStyle(
                              color: Color(0xff9598a4),
                              fontWeight: FontWeight.w500,
                              fontSize: 16,
            ...
            Padding(
              ...
              child: GestureDetector(
                ...
                child: MouseRegion(
                  cursor: SystemMouseCursors.click,
                  child: Text(
                    AppLocalizations.of(context).translate("show_use_terms"),
                    style: TextStyle(
                      color: Color(0xff64abff),
                      fontSize: 14,
                      fontWeight: FontWeight.w500,
