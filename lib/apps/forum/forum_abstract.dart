...
class ForumAbstractState extends State<ForumAbstract> {
  ...
  start() {
    if (_criteria != null) _getTopicList();
  }
  ...
  void initState() {
    ...
    if (_criteria != null) _newPostButtonEnabled = _criteria["forumId"] != null;
  }
  ...
  void didChangeDependencies() {
    ...
    _filters = [];
    _filters.add(DropdownMenuItem(
      child: Text(AppLocalizations.of(context).translate("app_forum_dropdown_value_0"), style: TextStyle(fontSize: 12.0, color: Color(0xFF111111), fontWeight: FontWeight.normal)),
      value: "",
    ));
    _filters.add(DropdownMenuItem(
      child: Text(AppLocalizations.of(context).translate("app_forum_dropdown_value_1"), style: TextStyle(fontSize: 12.0, color: Color(0xFF111111), fontWeight: FontWeight.normal)),
      value: "stickies",
    ));
    _filters.add(DropdownMenuItem(child: Text(AppLocalizations.of(context).translate("app_forum_dropdown_value_2"), style: TextStyle(fontSize: 12.0, color: Color(0xFF111111), fontWeight: FontWeight.normal)), value: "lastReply"));
    _filters.add(DropdownMenuItem(child: Text(AppLocalizations.of(context).translate("app_forum_dropdown_value_3"), style: TextStyle(fontSize: 12.0, color: Color(0xFF111111), fontWeight: FontWeight.normal)), value: "date"));
    ...
    if (widget.loadAuto && _criteria != null) _getTopicList();
  }
  ...
  _getTopicList({bool refresh = true}) async {
    ...
    if (_searchByValue != "") options["order"] = _searchByValue;
  ...
  _updatePageData() {
    ...
    if (_currentPage == _currentServicePage * _serviceRecsPerPageFactor && _topicsFetched.length <= _currentPage * _currentServicePage * _rowsPerPage) {
      print("reached Max");
  ...
  _onNewPostCloseHandler(dynamic retVal) {
    if (retVal != null) {
      if (retVal == "ok") {
        AlertManager.instance.showSimpleAlert(context: context, bodyText: AppLocalizations.of(context).translate("app_forum_submitOK"));
        _getTopicList();
      } else {
        print("error");
        print(retVal);
  ...
  _getTableViewActions(BuildContext context) {
    return Container(
        ...
        child: Row(
          ...
          children: [
            zDropdownButton(context, "", 220, _searchByValue, _filters, (value) {
              setState(() {
                _searchByValue = value;
                print("sort by " + _searchByValue.toString());
                if (_searchByValue != "") _getTopicList();
              });
            }),
            SizedBox(width: 10),
            Container(
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
                ...
                child: Center(
                  ...
                  Container(
                      ...
                      child: Row(
                        ...
                        crossAxisAlignment: CrossAxisAlignment.center,
                        children: [
                          ZButton(minWidth: 40, height: 40, key: _btnLeftKey, iconData: Icons.arrow_back_ios, iconColor: Colors.blue, iconSize: 30, clickHandler: _onPreviousPage, startDisabled: true),
                          Container(
                            height: 30,
                            width: 140,
                            child: Padding(
        ...
        _viewStatus == ViewStatus.topicView ? ForumTopicView(topicId: _selectedTopic, onReturnToForumView: _onReturnToForumView, myWidth: MediaQuery.of(context).size.width - 10, myHeight: widget.myHeight) : Container(),
        _showNewPost ? Center(child: ForumNewPost(parentSize: new Size(MediaQuery.of(context).size.width - 10, MediaQuery.of(context).size.height - 10), forumId: _criteria["forumId"], parent: null, onCloseBtnHandler: _onNewPostCloseHandler)) : Container(),
