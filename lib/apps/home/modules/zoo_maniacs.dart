...
class HomeModuleManiacsState extends State<HomeModuleManiacs>{
  ...
  _getTopLevels() async {
    var res = await _rpc.callMethod("Points.Main.getUsersByLevel", [{"recsPerPage" : 4, "page": 1}]);
    ...
    if (res["status"] == "ok"){
      ...
      setState(() {
        for (int i=0; i<records.length; i++){
          _levelUsers.add(getLevelManiacItem(LevelManiacRecord.fromJSON(records[i]),i));
        }
      });
  ...
  _getTopPoints() async {
    var res = await _rpc.callMethod("Points.Main.getUsersByPoints", [{"recsPerPage" : 4, "page": 1}]);
    ...
    if (res["status"] == "ok"){
      ...
      setState(() {
        for (int i=0; i<records.length; i++){
          _pointsUsers.add(getPointManiacItem(PointsManiacRecord.fromJSON(records[i]), i));
        }
      });
    } else {
            ...
            child: Row(
              ...
              children: [
                Padding(
                  ...
                  child: Text((index+1).toString(), style: TextStyle(color: Color(0xffBFC1C4), fontSize: 20, fontWeight: FontWeight.bold),
                  textAlign: TextAlign.left)
                ),
                ClipOval(
                  child: _hasMainPhoto ?
                    Image.network(Utils.instance.getUserPhotoUrl(photoId: data.user.mainPhoto["image_id"].toString()),
                        height: 45,
                        width: 45,
                        fit: BoxFit.contain) :
                    Image.asset(data.user.sex == 1 ?  "assets/images/home/maniac_male.png" : "assets/images/home/maniac_female.png",
                        height: 45,
                        width: 45,
                        fit: BoxFit.contain),
                  ),
                Container(width: _usernameFieldWidth, margin: EdgeInsets.only(left: 5), child:
                Text(data.user.username, style: TextStyle(color:  Color(0xffFF9C00), fontSize: 15), overflow: TextOverflow.ellipsis,
                    maxLines: 1)),
                ...
                child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Container(width: _usernameFieldWidth, margin: EdgeInsets.only(left: 5), child: Text(data.user.username, style: TextStyle(color:  Color(0xffFF9C00), fontSize: 15), overflow: TextOverflow.ellipsis,
                          maxLines: 1)),
                        Padding(padding: EdgeInsets.symmetric(horizontal: 5), child: Image.asset("assets/images/home/star.png")),
                        Expanded(child: Container()),
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: Center(
                    child: Image.asset("assets/images/home/zoomaniacs.png")
                  )
                ),
                Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            Padding(
                              ...
                              child: Text(AppLocalizations.of(context).translate("app_home_module_zoo_maniacs_weekly_zpoints"),
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            Padding(
                              padding: EdgeInsets.symmetric(vertical: 3),
                              child: Text(AppLocalizations.of(context).translate("app_home_module_zoo_maniacs_weekly_zlevel"),
                                  style: TextStyle(color: Colors.black, fontSize: 18), textAlign: TextAlign.left),
                            ),
                            Container(
                              child: Column(
