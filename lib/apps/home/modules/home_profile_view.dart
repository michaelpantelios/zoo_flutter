...
class HomeModuleProfileViewState extends State<HomeModuleProfileView>{
                      ...
                      children: [
                        ...
                        Container(width: _usernameFieldWidth, margin: EdgeInsets.only(left: 5), child:
                          Text(data.user.username, style: TextStyle(color:  Color(0xffFF9C00), fontSize: 15), overflow: TextOverflow.ellipsis,
                            maxLines: 1)),
                        Text(AppLocalizations.of(context).translate("app_home_module_profileViews_label"),
    ...
    if (res["status"] == "ok") {
      ...
      setState(() {
        for(int i=0; i<res["data"]["viewDates"].length; i++){
          _dates.add(res["data"]["viewDates"][i].toString());
        }
    ...
    if (res["status"] == "ok") {
      ...
      setState(() {
        ...
        for(int i=0; i<res["data"].length; i++){
          if (i<3)
           _viewersList.add(getViewerItem(StatsProfileViewRecord.fromJSON(res["data"][i])));
        }
      });
    } else {
      print("ERROR");
  ...
  _onDateChanged(String date){
    if (date != _newestDateString){
      PopupManager.instance.show(context: context, options: CostTypes.oldStats, popup: PopupType.Protector, callbackAction: (retVal)=>{
        if (retVal == "ok")
          getProfileViewsForDate(date: date, pay: 1)
      });
    } else getProfileViewsForDate(date: date, pay: 0);
  }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                zDropdownButton(
                    ...
                    _selectedDateString,
                      ...
                      return DropdownMenuItem<String>(
                        value: value,
                        child: Text(Utils.instance.getNiceForumDate(dd: value, hours: false),
                ...
                SizedBox(height:10),
                _noViews ? Center (
                  child: Text(AppLocalizations.of(context).translate("app_home_module_profileViews_noViews"), style: TextStyle())
                ) :
                Container(
                  width: _itemWidth,
                  height: 180,
