  ...
  getHotTopics() async {
    ...
    if (res["status"] == "ok"){
      setState(() {
        ...
        for(int j = 0; j < 3; j++){
          if (j<= _topics.length-1)
            _hotTopicsItems.add(getTopicItem(_topics[j], j));
        }
      });
    } else {
      print("error");
  ...
  getTopicItem(ForumTopicRecordModel info, int index){
    return Column(
      children: [
        Row(
          children: [
             Text(AppLocalizations.of(context).translate("app_home_module_forum_hot_subject"),
        ...
        Row(
          children: [
            Text(AppLocalizations.of(context).translate("app_home_module_forum_hot_from"),
                  style: TextStyle(color: Colors.black, fontSize: 12, fontWeight: FontWeight.normal),
                  textAlign: TextAlign.left),
            SizedBox(width: 3),
            Flexible(
        ...
        Row(
          children: [
            Text(AppLocalizations.of(context).translate("app_home_module_forum_hot_date"),
                style: TextStyle(color: Colors.black, fontSize: 12, fontWeight: FontWeight.normal),
                textAlign: TextAlign.left),
            SizedBox(width: 3),
            Text(Utils.instance.getNiceForumDate(dd : info.date), style: TextStyle(color: Colors.black, fontWeight:FontWeight.bold ),
                textAlign: TextAlign.left)
          ],
        ),
        Row(
          children: [
            Text(AppLocalizations.of(context).translate("app_home_module_forum_hot_replies"),
                style: TextStyle(color: Colors.black, fontSize: 12, fontWeight: FontWeight.normal),
                textAlign: TextAlign.left),
            SizedBox(width: 3),
            Text(info.repliesNo.toString(), style: TextStyle(color: Colors.black, fontWeight:FontWeight.bold ),
                textAlign: TextAlign.left)
          ],
        ),
        if (index < 2)  Padding(
          padding: EdgeInsets.symmetric(vertical: 3),
          child: Divider( height: 1,color: Color(0xffD2D2D2))
          ) else Container()
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          getModuleHeader(AppLocalizations.of(context).translate("app_home_module_title_forum_hot")),
          Container(
