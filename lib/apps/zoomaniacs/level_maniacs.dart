...
import 'package:zoo_flutter/apps/zoomaniacs/level_maniacs_item.dart';
...
class LevelManiacs extends StatefulWidget{
  LevelManiacs({this.myHeight});

  final double myHeight;

  LevelManiacsState createState() => LevelManiacsState();
}

class LevelManiacsState extends State<LevelManiacs>{
  LevelManiacsState();
  ...
  _getUsersByPoints({bool addMore = false}) async {
    ...
    var res = await _rpc.callMethod("Points.Main.getUsersByLevel", [options]);
    ...
    if(res["status"] == "ok"){
      ...
      for(int i=0; i<records.length; i++){
        LevelManiacRecord record = LevelManiacRecord.fromJSON(records[i]);
                    ...
                    Container(
                        ...
                        child:  Row(
                          ...
                          children: [
                            ...
                            SizedBox(width: 10),
                            Text(AppLocalizations.of(context).translateWithArgs("app_zoomaniacs_my_level", [UserProvider.instance.userInfo.level.toString()]),
